function [matUv_opt, obj_Phi_1_opt, obj_Omega_opt] = ...
    optimizeUv_OGM(Delta_v, vecYv, mat_tenCv, mat_tenBv, matUv, vecNormDDv, para)
% -------------------------------------------------------------------------
...
% Pre-calculation
% -------------------------------------------------------------------------
pre_calc.vecNormDDv = vecNormDDv; clear vecNormDDv
pre_calc.matBBv = mat_tenBv * mat_tenBv';
pre_calc.normBBv = norm(pre_calc.matBBv, 2);
pre_calc.matCBv = mat_tenCv * mat_tenBv';
pre_calc.normCv = norm(mat_tenCv,'fro')^2;

% -------------------------------------------------------------------------
% Initialization of the objective, gradient, and Lipschitz constant
% -------------------------------------------------------------------------
[obj(1,1), grad, lipsc, obj_Phi_1, obj_Omega] = ...
    evaluate_cost(Delta_v, vecYv, mat_tenCv, mat_tenBv, matUv, mu, rho, pre_calc, para);
...
while loop
    ...
    [obj(t,1), grad_new, lipsc_new, obj_Phi_1_new, obj_Omega_new] = ...
        evaluate_cost(Delta_v, vecYv, mat_tenCv, mat_tenBv, matUv_new, mu, rho, pre_calc, para);
...
clear vecUD vecZv vecKv coeff


% obj_Phi_temp2 = norm((mat_tenCv-matUv*mat_tenBv), 'fro')^2;
obj_Phi_temp2 = pre_calc.normCv - 2.0*trace(matUv'*pre_calc.matCBv) ...
    + trace(pre_calc.matBBv*(matUv'*matUv));

% grad_Phi_temp2 = matUv*pre_calc.matBBv - mat_tenCv*mat_tenBv';
grad_Phi_temp2 = matUv*pre_calc.matBBv - pre_calc.matCBv;
